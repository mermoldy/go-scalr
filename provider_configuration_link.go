...
type ProviderConfigurationLinks interface {
	WorkspaceList(ctx context.Context, workspaceID string, options ProviderConfigurationLinksListOptions) (*ProviderConfigurationLinksList, error)
	WorkspaceCreate(ctx context.Context, workspaceID string, options ProviderConfigurationWorkspaceLinkCreateOptions) (*ProviderConfigurationLink, error)
	EnvironmentCreate(ctx context.Context, environmentID string, options ProviderConfigurationEnvironmentLinkCreateOptions) (*ProviderConfigurationLink, error)
...
// List all provider configuration links within the workspace.
func (s *providerConfigurationLinks) WorkspaceList(ctx context.Context, workspaceID string, options ProviderConfigurationLinksListOptions) (*ProviderConfigurationLinksList, error) {
...
// ProviderConfigurationWorkspaceLinkCreateOptions represents the options for creating a new provider configuration workspace link.
type ProviderConfigurationWorkspaceLinkCreateOptions struct {
	ID    string  `jsonapi:"primary,provider-configuration-links"`
	Alias *string `jsonapi:"attr,alias"`
...
// WorkspaceCreate is used to create a new provider configuration workspace link.
func (s *providerConfigurationLinks) WorkspaceCreate(ctx context.Context, workspaceID string, options ProviderConfigurationWorkspaceLinkCreateOptions) (*ProviderConfigurationLink, error) {
	options.ID = ""
...
// ProviderConfigurationEnvironmentLinkCreateOptions represents the options for creating a new provider configuration link to environment.
type ProviderConfigurationEnvironmentLinkCreateOptions struct {
	ID      string `jsonapi:"primary,provider-configuration-links"`
	Default *bool  `jsonapi:"attr,default"`
...
// WorkspaceCreate is used to create a new provider configuration environment link.
func (s *providerConfigurationLinks) EnvironmentCreate(ctx context.Context, environmentID string, options ProviderConfigurationEnvironmentLinkCreateOptions) (*ProviderConfigurationLink, error) {
	options.ID = ""

	url_path := fmt.Sprintf("environments/%s/provider-configuration-links", url.QueryEscape(environmentID))
	req, err := s.client.newRequest("POST", url_path, &options)
	if err != nil {
		return nil, err
	}
